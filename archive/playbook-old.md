...
|---|---|---|
| `Pending` | Not scheduled — resource pressure, unbound PVC, or node taint | [Pending sub-branch](#sub-pending) |
| `Init:0/1` or `Init:CrashLoopBackOff` | Init container has not completed or is failing | [Init Container sub-branch](#sub-init) |
| `ImagePullBackOff` / `ErrImagePull` / `ErrImageNeverPull` | Image cannot be pulled | [ImagePull sub-branch](#sub-imagepull) |
| `CrashLoopBackOff` | Container starts and immediately exits repeatedly | [CrashLoop sub-branch](#sub-crashloop) |
| `Running` but READY shows `0/1` | Container is up but failing readiness probe | [Readiness sub-branch](#sub-readiness) |
| `Running` with RESTARTS climbing | Container killed and restarted by liveness probe | [Liveness sub-branch](#sub-liveness) |
...
**In `kubectl describe pod <pod> -n <ns>` — Events section:** Healthy: event reason `Pulled` with a valid image string. Broken: `Failed to pull image "myapp:badtag"` with reason `ErrImagePull` — the exact image string it tried is in this message. Compare against the known-good image from the old pod or rollout history (see below).

**In `kubectl describe pod <pod> -n <ns>` — Containers → Image field:** Shows the full image string the pod is configured to use. Broken: typo in registry/repo name (e.g. `ngixn` instead of `nginx`), nonexistent tag, or missing registry prefix.

**In `kubectl describe pod <pod> -n <ns>` — Events mentioning auth:** Broken: `unauthorized: authentication required` or `pull access denied` — the image exists but credentials are missing. Check `imagePullSecrets` on the pod spec (`kubectl get pod <pod> -n <ns> -o jsonpath='{.spec.imagePullSecrets}'`) and whether the referenced Secret exists (`kubectl get secret -n <ns>`).
...
**When logs show `connection refused`, `ECONNREFUSED`, or `Name or service not known` to a dependency:** the dependency hostname or port is wrong in the pod's env. Find correct values with `kubectl get svc -n <ns>` and compare against `kubectl exec <pod> -n <ns> -- env | sort`. Fix the ConfigMap/Secret (see [Bucket F](#bucket-f)), then `kubectl rollout restart deploy/<deploy> -n <ns>`. Verify: pod stays Running with no restarts climbing.

**When logs show `password authentication failed` or `database does not exist`:** wrong credential or DB name in config. Find correct values by comparing against the authoritative ConfigMap/Secret (see [Bucket F](#bucket-f) or [Bucket H](#bucket-h)). Fix: `kubectl edit configmap <cm> -n <ns>` or `kubectl edit secret <secret> -n <ns>`, then `kubectl rollout restart deploy/<deploy> -n <ns>`. Verify: pod starts cleanly, logs show no auth errors.

**When exit code is 137 (OOMKilled):** container exceeded memory limit. Find current limit: `kubectl get pod <pod> -n <ns> -o jsonpath='{.spec.containers[0].resources.limits.memory}'`. Fix: `kubectl edit deploy <deploy> -n <ns>` — increase `resources.limits.memory`. Verify: pod stays Running, no OOMKill in `describe pod` Last State.

**When logs show a stack trace or panic on startup:** application code is crashing. If a recent image change caused it: `kubectl rollout undo deploy/<deploy> -n <ns>`. Verify: pod starts cleanly with the previous image.

**When the container exits immediately with no log output:** bad `command` or `args` override in the pod spec. Check: `kubectl get deployment <deploy> -n <ns> -o yaml | grep -A 5 "command\|args"`. Fix: `kubectl edit deployment <deploy> -n <ns>` — correct or remove the override. Verify: pod starts and logs appear.

**When `describe pod` shows probe failures causing restarts:** wrong probe path or port. Fix: `kubectl edit deploy <deploy> -n <ns>` — correct the probe (see [Readiness sub-branch](#sub-readiness) or [Liveness sub-branch](#sub-liveness)). Verify: restarts stop climbing.
...
If you get a 404, try common health endpoint paths: `/health`, `/healthz`, `/ready`, `/readyz`, `/api`, `/`. Also check the app's startup logs — many frameworks print their registered routes on boot: `kubectl logs <pod> -n <ns> | head -30`. If an old working pod exists (e.g., during a stuck rollout), compare its probe config as the source of truth — see the comparison step below.
...
**When `describe pod` shows `Readiness probe failed: HTTP probe failed with statuscode: 404`:** the probe path doesn't match any app route. Find a valid path using the curl discovery step above, or compare against the old pod's probe. Fix: `kubectl edit deploy <deploy> -n <ns>` — correct `readinessProbe.httpGet.path`. Verify: pod becomes `1/1` Ready.

**When `describe pod` shows `Readiness probe failed: dial tcp ... connection refused`:** the probe port doesn't match the container's listening port. Find the correct port: `kubectl get pod <pod> -n <ns> -o jsonpath='{.spec.containers[0].ports[0].containerPort}'`. Fix: `kubectl edit deploy <deploy> -n <ns>` — correct `readinessProbe.httpGet.port`. Verify: pod becomes Ready.

**When the probe fails only in the first few seconds then succeeds:** `initialDelaySeconds` is too short — the app needs more time to start. Fix: `kubectl edit deploy <deploy> -n <ns>` — increase `readinessProbe.initialDelaySeconds`. Verify: pod reaches Ready without probe failure events.

**When the probe path and port are correct but the app returns 5xx:** the app itself is unhealthy. Check logs — go to [Bucket H](#bucket-h).
...
You are done when the pod becomes `1/1` Ready and endpoints populate.
...
**When `describe pod` shows `Liveness probe failed` and curl to the probe path returns 404:** the probe path is wrong. Find a valid path using the same approach as [Readiness sub-branch](#sub-readiness) — port-forward and curl. Fix: `kubectl edit deploy <deploy> -n <ns>` — correct `livenessProbe.httpGet.path`. Verify: restart count stops climbing.

**When `describe pod` shows `Liveness probe failed: dial tcp ... connection refused`:** the probe port is wrong. Find the correct port: `kubectl get pod <pod> -n <ns> -o jsonpath='{.spec.containers[0].ports[0].containerPort}'`. Fix: `kubectl edit deploy <deploy> -n <ns>` — correct `livenessProbe.httpGet.port`. Verify: restart count stops climbing.

**When the probe path and port are correct and curl works manually, but the pod still restarts:** the probe timing is too aggressive — the app is responding correctly but not fast enough. Fix: `kubectl edit deploy <deploy> -n <ns>` — increase `timeoutSeconds` (default is 1, often too short), `periodSeconds`, or `failureThreshold` (how many consecutive failures before kill). Verify: watch `kubectl get pods -n <ns> -w` — restart count stops climbing.

**When the app genuinely becomes unhealthy under load:** the liveness probe is doing its job — killing an unhealthy container. The fix is in the application, not the probe. Check logs for the root cause.
...
Check the new RS's pods — they usually have a clear error. If the new RS pods show `ImagePullBackOff`, go to the [ImagePull sub-branch](#sub-imagepull). If they show `CrashLoopBackOff`, go to the [CrashLoop sub-branch](#sub-crashloop). If they show `Running` but `0/1` Ready, go to the [Readiness sub-branch](#sub-readiness). If the issue is update/rollback behavior rather than a pod-level failure, stay here.
...
**Selector mismatch (endpoints empty):** Find correct label: `kubectl get pods -n <ns> --show-labels`. Fix: `kubectl patch svc <svc> -n <ns> -p '{"spec":{"selector":{"app":"<correct-label>"}}}'`. Verify: `kubectl get endpoints <svc> -n <ns>` shows pod IPs, then `curl -s localhost/` returns a valid response through Ingress.

**Wrong targetPort:** Find correct port: `kubectl describe pod <pod> -n <ns>` → `Containers: → Ports: → ContainerPort`. Fix: `kubectl patch svc <svc> -n <ns> -p '{"spec":{"ports":[{"port":<svc-port>,"targetPort":<correct-port>}]}}'`. Verify: `kubectl port-forward svc/<svc> 8080:<svc-port> -n <ns>` then `curl localhost:8080/` returns valid response, then `curl -s localhost/` works through Ingress.

**Readiness probe failing (pods not Ready → empty endpoints):** Fix the probe in [Bucket B](#bucket-b) first — once pods become Ready, endpoints auto-populate. Verify: `curl -s localhost/` returns a valid response end-to-end.

**NetworkPolicy blocking traffic:** Confirm by temporarily deleting suspect policy: `kubectl delete networkpolicy <policy> -n <ns>` then `curl localhost/`. If traffic works, re-apply a corrected policy. See [Bucket I](#bucket-i). Verify: `curl -s localhost/` works after re-applying the corrected policy.
...
kubectl port-forward svc/<svc> 8080:<svc-port> -n <ns>
curl -i http://localhost:8080
# Always finish with external end-to-end test:
curl -s localhost/
curl -s localhost/health
curl -s localhost/items
```

Healthy: endpoints populated, port-forward curl returns valid JSON, **and** external curl through Ingress returns valid responses on all endpoints. Do not stop at port-forward — that only proves the Service layer. The external curl proves the full path.
...
| App returns 404 on requests that should work | Wrong base path, route config, or URL prefix flag | `kubectl exec <pod> -n <ns> -- wget -qO- http://localhost:<port>/` to discover what paths exist; check env vars for `BASE_PATH` or `ROOT_PATH`; check startup logs for registered routes (`kubectl logs <pod> -n <ns> \| head -30`) | Compare against working pod's env or app documentation/Dockerfile |
