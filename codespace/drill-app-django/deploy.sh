...
kubectl rollout status deployment/incident-db -n "$NAMESPACE" --timeout=90s

echo "=== Waiting for API ==="
kubectl rollout status deployment/incident-api -n "$NAMESPACE" --timeout=120s
...
for i in $(seq 1 10); do
  ...
  if [ "$STATUS" = "200" ]; then
    ...
    echo ""
    curl -s http://localhost/api/v1/incidents | python3 -m json.tool 2>/dev/null || curl -s http://localhost/api/v1/incidents
    echo ""
